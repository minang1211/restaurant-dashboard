...
function ColumnFilter({ label, options, selected, onToggle, onToggleAll }) {
    ...
    return (
        <th className="p-4 font-bold border-r border-[#1a2d4a] relative" ref={ref}>
            <button onClick={() => setOpen(!open)} className="flex items-center gap-1 text-[#f97316] hover:text-white transition-colors">
                {label}
                ...
                <div className="absolute left-0 top-full mt-1 w-48 bg-[#0f1d32] border border-[#1a2d4a] rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
                    ...
                    {options.map((opt) => (
                        <button key={opt.value} onClick={() => onToggle(opt.value)} className="w-full text-left px-3 py-2 text-xs font-bold text-gray-300 hover:bg-[#1a2d4a] flex items-center gap-2">
                            <span className={`w-3.5 h-3.5 rounded border flex items-center justify-center text-[10px] ${selected.includes(opt.value) ? "bg-[#f97316] border-[#f97316] text-white" : "border-gray-500"}`}>
    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-[#0f1d32] border border-[#f97316] rounded-lg px-4 py-2 shadow-lg">
                ...
                <p className="text-[#f97316] font-bold">{payload[0].value} calls ({((payload[0].value / callLogs.length) * 100).toFixed(1)}%)</p>
            </div>
        );
    }
    return null;
            ...
            <div className="bg-[#0f1d32] rounded-[2rem] p-8 shadow-inner border border-[#1a2d4a]">
                ...
                <div className="flex justify-end mb-6">
                    <div className="relative" ref={filterRef}>
                        ...
                        {filterOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-[#0f1d32] border border-[#1a2d4a] rounded-lg shadow-lg z-50 overflow-hidden">
                                {filterOptions.map((option) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2.5 text-sm font-bold transition-colors ${filter === option ? "bg-[#f97316] text-white" : "text-gray-300 hover:bg-[#1a2d4a] hover:text-[#f97316]"
                                            }`}
                                    >
                                        {option}
                                    </button>
                                ...
                                {filter === "Custom" && (
                                    <div className="px-4 py-3 border-t border-[#1a2d4a] space-y-2">
                                        ...
                                        <button onClick={() => setFilterOpen(false)} className="w-full bg-[#f97316] text-white text-xs font-bold py-1.5 rounded hover:bg-[#ea580c] transition-colors mt-1">
                                            Apply
                                        </button>
                                    </div>
                                )}
                                ...
                                <PieChart>
                                    ...
                                    >
                                        {pieData.map((entry, index) => (
                                            <Cell
                                                key={`cell-${index}`}
                                                fill={activeIndex === index ? "#f97316" : entry.fill}
                ...
                <div className="bg-[#1a2d4a]/50 rounded-3xl overflow-visible">
                    ...
                    {tableData.length > showCount && (
                        <div className="p-4 text-center border-t border-[#1a2d4a]">
                            <button onClick={() => setShowCount(showCount + 5)} className="bg-[#1a2d4a] hover:bg-[#f97316] text-white font-bold py-2 px-6 rounded-full transition-colors text-sm">
                                View More
                            </button>
                        </div>
                    )}
